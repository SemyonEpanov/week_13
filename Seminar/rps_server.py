...
class RockPaperScissorsGame:
    ...
    async def determine_winner(self):
        ...
        await self.ask_for_rematch({"type": "rematch", "message": "Хотите сыграть еще раз? (yes, no)"})
    ...
    async def ask_for_rematch(self, message):
        """
        Отправляет запрос на
        переигровку обоим игрокам, собирает их ответы и
        решает: перезапустить или завершить игру
        """
        await self.player1_ws.send(json.dumps(message))
        await self.player2_ws.send(json.dumps(message))
        # Получает ответы от обоих игроков
        responses = await asyncio.gather(
            self.receive_response(self.player1_ws),
            self.receive_response(self.player2_ws),
        )
        # Если оба игрока согласны на переигровку, начинает новую игру
        if all(responses):
            self.game_over = False
            await start_game(self)
        else:
            # Отправляет сообщение об окончании игры
            await self.broadcast(
                {"type": "end", "message": "Игра окончена. Спасибо за игру!"}
            )
    ...
    async def receive_response(self, websocket):
        ...
        Возвращает:
            bool: True, если игрок согласен на переигровку, иначе False.
        """
        try:
            data = await websocket.recv()  # Получает данные от игрока
            message = json.loads(data)  # Парсит JSON сообщение
            return message.get("rematch", False)
        except websockets.exceptions.ConnectionClosed:
            # Обрабатывает случай, когда игрок отключается
            print("Соединение закрыто")
            return False
    ...
    while not game.game_over:
        ...
        await game.player1_ws.send(
            json.dumps(
                {
                    "type": "your_move",
                    "message": "Введите ваш ход (rock, paper, scissors):",
                }
            )
        )
        await game.player2_ws.send(
